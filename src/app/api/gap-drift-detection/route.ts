...
// AI gap: Drift/Regression Detection (scale)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'scale Gap drift-detection' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You detect model drift and regression in evaluation runs.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'drift-detection', payload: { keys: Object.keys(body) } });
...
export async function GET() {
  return NextResponse.json({ feature: 'drift-detection', events: gapFeatures.filter(r => r.slug === 'drift-detection').length });
}

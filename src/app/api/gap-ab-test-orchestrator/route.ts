...
// AI gap: A/B Test Orchestrator on Evaluations (scale)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'scale Gap ab-test-orchestrator' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You design A/B test plans over evaluation results.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'ab-test-orchestrator', payload: { keys: Object.keys(body) } });
    return NextResponse.json({ plan: out });
  } catch (e: any) {
...
export async function GET() {
  return NextResponse.json({ feature: 'ab-test-orchestrator', events: gapFeatures.filter(r => r.slug === 'ab-test-orchestrator').length });
}

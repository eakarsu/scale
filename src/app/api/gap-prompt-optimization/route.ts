...
// AI gap: Prompt Optimization Loop (scale)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'scale Gap prompt-optimization' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You compare prompt variations, score them, and return the best with rationale.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'prompt-optimization', payload: { keys: Object.keys(body) } });
    return NextResponse.json({ best: out });
  } catch (e: any) {
...
export async function GET() {
  return NextResponse.json({ feature: 'prompt-optimization', events: gapFeatures.filter(r => r.slug === 'prompt-optimization').length });
}
